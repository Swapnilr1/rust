...
fn codegen_msvc_try<'ll>(
    ...
    let (llty, llfn) = get_rust_try_fn(bx, &mut |mut bx| {
        ...
        let slot = bx.alloca(bx.type_i8p(), ptr_align, false, false);
        let try_func_ty = bx.type_func(&[bx.type_i8p()], bx.type_void());
...
fn codegen_emcc_try<'ll>(
    ...
    let (llty, llfn) = get_rust_try_fn(bx, &mut |mut bx| {
        ...
        let catch_data = bx.alloca(catch_data_type, ptr_align, false, false);
        let catch_data_0 =
    ...
    if name == sym::simd_bitmask {
        ...
        match ret_ty.kind() {
                        ...
                        == Some(expected_bytes) =>
            {
                ...
                let ptr = bx.alloca(bx.type_ix(expected_bytes * 8), Align::ONE, false, false);
